...
export default function CurriculumCoverage() {
  const cards: CardSpec[] = [
    ...
    },
    {
      ...
      abbr: 'WA',
      name: 'SCSA',
      sub: 'Western Australian Curriculum',
      color: 'purple',
    },
  ]
    ...
    <section className='bg-gradient-to-b from-rose-50 to-amber-50 py-16 px-6 md:px-12'>
          ...
          return (
            ...
            >
              <div className='flex flex-col items-center mb-3'>
                <span
                  className={`h-16 w-16 rounded-full border-4 ${cc.circleBorder} ${cc.circleBg} grid place-content-center text-lg font-bold ${cc.circleText} mb-2 transition-transform group-hover:scale-110`}
                >
                  {c.abbr}
                </span>
              ...
              {c.viewMapping && (
                <span
                  className={`inline-flex items-center gap-2 ${cc.linkText} font-semibold group-hover:underline`}
                >
                  View mapping{' '}
              ...
              <div className='mt-4 flex justify-center'>
                <span
                  className={`flex items-center gap-1 ${cc.verifiedText} text-xs font-semibold`}
                >
      ...
      <div className='max-w-6xl mx-auto mb-12'>
        <div className='rounded-xl bg-gradient-to-r from-lime-100 via-emerald-50 to-lime-100 border border-lime-300 px-6 py-4 text-center text-sm text-gray-800 shadow-sm font-medium'>
          <span className='font-bold text-lime-700'>Auto-mapped</span> standards
          • State-specific language • 100% alignment
        </div>
      </div>
