...
export default function SubjectDetailPage() {
  ...
  const [version, setVersion] = useState<'1.0' | '2.0'>(
    (searchParams?.get('version') as '1.0' | '2.0') || '1.0'
  )
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const stds = await CurriculumService.getAchievementStandards(
          subjectName,
          level
        )
    ...
    <div className='min-h-screen bg-gradient-to-br from-[#FDE5DA] via-white to-[#FFF2E8]'>
      <div className='container mx-auto px-4 py-8'>
        ...
        <div className='text-center mb-8'>
          ...
          <h1 className='text-4xl font-bold text-gray-900 font-fredoka'>
            {subjectName}
          </h1>
          <p className='text-gray-600 font-nunito mt-2'>
            Content descriptions with linked achievement standards
          </p>
        </div>
        ...
        {error && (
          <div className='max-w-3xl mx-auto mb-6 p-4 bg-red-50 text-red-700 border border-red-200 rounded-lg text-sm'>
            {error}
          </div>
        )}
              ...
              <Card key={strand} className='shadow-sm'>
                <CardHeader>
                  <div className='flex items-center justify-between'>
                    ...
                    >
                      <Target className='w-3 h-3' /> {items.length}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className='space-y-4'>
                    {items.map((it) => (
                      <div
                        key={
                          (it['Code'] || (it as any).code || it.id) as string
                        }
                        className='rounded-lg border p-4'
                      >
                        <div className='flex items-center justify-between'>
                          <div className='text-sm font-semibold'>
                            {(it['Code'] || (it as any).code) as string}
                          </div>
                        ...
                        <div className='mt-2 text-sm text-gray-800'>
                          {
                            (it['Content Description'] ||
                              (it as any).content_description ||
                              it['Description'] ||
                              (it as any).description) as string
                          }
                        </div>
                        ...
                        {standards.length > 0 && (
                          <div className='mt-3 bg-gray-50 border rounded-md p-3'>
                            ...
                            <ul className='list-disc list-inside text-xs text-gray-700 space-y-1'>
                              {relatedStandards(it['Strand'] as any)
                                .slice(0, 3)
                                .map((st, idx) => (
                                  <li
                                    key={
                                  ...
                                  >
                                    {
                                      (st['Description'] ||
                                        ...
                                        st['Achievement Standard'] ||
                                        (st as any)
                                          .achievement_standard) as string
                                    }
                                  </li>
                                ))}
                            </ul>
