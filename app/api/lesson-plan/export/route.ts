...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!lessonPlan || (format !== 'pdf' && format !== 'docx')) {
      return NextResponse.json({ error: 'Missing lessonPlan or invalid format' }, { status: 400 })
    }
    ...
    if (format === 'pdf') {
      ...
      doc.moveDown().fontSize(12).text(lp.overview || '')
      ...
      if (lp.linkedCurriculum?.length) {
        ...
        lp.linkedCurriculum.forEach((c: any) => doc.fontSize(12).text(`• ${c.code}: ${c.description}`))
      }
      ...
      if (lp.lessonTimeline?.length) {
        ...
        lp.lessonTimeline.forEach((i: any) => {
          ...
          if (i.teacherMoves?.length) doc.text(` Teacher: ${i.teacherMoves.join(', ')}`)
          if (i.studentTasks?.length) doc.text(` Students: ${i.studentTasks.join(', ')}`)
        })
      }
      ...
      if (lp.traumaInformedStrategies?.length) {
        ...
        lp.traumaInformedStrategies.forEach((t: string) => doc.fontSize(12).text(`• ${t}`))
      }
      ...
      if (lp.assessment) {
        ...
        if (a.methods?.length) doc.fontSize(12).text(`Methods: ${a.methods.join(', ')}`)
      ...
      if (lp.differentiation) {
        ...
        if (d.access?.length) doc.fontSize(12).text(`Access: ${d.access.join(', ')}`)
        if (d.extension?.length) doc.fontSize(12).text(`Extension: ${d.extension.join(', ')}`)
      }
      ...
      sections: [
        {
          children: [
            ...
            ...(lp.learningGoals?.length ? [
              new Paragraph({ text: 'Learning Goals', heading: HeadingLevel.HEADING_2 }),
              ...lp.learningGoals.map((g: string) => new Paragraph(`• ${g}`))
            ] : []),
            ...(lp.linkedCurriculum?.length ? [
              new Paragraph({ text: 'Linked Curriculum', heading: HeadingLevel.HEADING_2 }),
              ...lp.linkedCurriculum.map((c: any) => new Paragraph(`• ${c.code}: ${c.description}`))
            ] : []),
            ...(lp.lessonTimeline?.length ? [
              new Paragraph({ text: 'Lesson Timeline', heading: HeadingLevel.HEADING_2 }),
              ...lp.lessonTimeline.map((i: any) => new Paragraph(`${i.minutes} min – ${i.activity}`))
            ] : []),
            ...(lp.resources?.length ? [
              new Paragraph({ text: 'Resources', heading: HeadingLevel.HEADING_2 }),
              ...lp.resources.map((r: string) => new Paragraph(`• ${r}`))
            ] : []),
            ...(lp.traumaInformedStrategies?.length ? [
              new Paragraph({ text: 'Trauma-Informed Strategies', heading: HeadingLevel.HEADING_2 }),
              ...lp.traumaInformedStrategies.map((t: string) => new Paragraph(`• ${t}`))
            ] : []),
            ...(lp.indigenousPerspectives ? [
              new Paragraph({ text: 'Indigenous Perspectives', heading: HeadingLevel.HEADING_2 }),
              new Paragraph(lp.indigenousPerspectives)
            ] : []),
            ...(lp.assessment ? [
              new Paragraph({ text: 'Assessment', heading: HeadingLevel.HEADING_2 }),
              ...(lp.assessment.style ? [new Paragraph(`Style: ${lp.assessment.style}`)] : []),
              ...(lp.assessment.methods?.length ? [new Paragraph(`Methods: ${lp.assessment.methods.join(', ')}`)] : []),
              ...(lp.assessment.successCriteria?.length ? [
                new Paragraph('Success Criteria:'),
                ...lp.assessment.successCriteria.map((s: string) => new Paragraph(`• ${s}`))
              ] : [])
            ] : []),
            ...(lp.differentiation ? [
              new Paragraph({ text: 'Differentiation', heading: HeadingLevel.HEADING_2 }),
              ...(lp.differentiation.access?.length ? [new Paragraph(`Access: ${lp.differentiation.access.join(', ')}`)] : []),
              ...(lp.differentiation.extension?.length ? [new Paragraph(`Extension: ${lp.differentiation.extension.join(', ')}`)] : []),
            ] : []),
            ...(lp.notesForTeacher ? [
              new Paragraph({ text: 'Notes for Teacher', heading: HeadingLevel.HEADING_2 }),
              new Paragraph(lp.notesForTeacher)
            ] : []),
            ...(lp.complianceSummary ? [
              new Paragraph({ text: 'Compliance Summary', heading: HeadingLevel.HEADING_2 }),
