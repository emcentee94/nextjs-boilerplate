...
const supabaseUrl =
  ...
  'https://kpdusbhqiswdiyzdwxpw.supabase.co'
const supabaseKey =
...
if (!supabaseKey) {
  console.warn(
    'Missing Supabase environment variables - using file-based storage only'
  )
}
  ...
  try {
    ...
    if (isProduction) {
      ...
      if (supabase) {
        ...
        if (insertError) {
          console.warn(
            'Supabase insert failed in dev; continuing with file log:',
            insertError
          )
        }
      }
...
export async function GET() {
  try {
    ...
    const sortedRequests = demoRequests.sort(
      (a, b) =>
