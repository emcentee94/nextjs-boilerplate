...
export class VCAACurriculumPDFGenerator {
  ...
  constructor() {
    ...
    this.outputPath = path.join(process.cwd(), 'public', 'resources', 'vcaa-curriculum-complete-guide.pdf');
  }
  ...
  generateCurriculumPDF(data: VCAACurriculumData): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      this.doc.fontSize(20)
             .text('Version 2.0', 50, 130, { align: 'center' });
      ...
      this.addContent('The Victorian Curriculum F-10 includes eight key learning areas:');
      this.addBulletList(data.learningAreas);
      this.doc.addPage();

      // Capabilities
      this.addHeader('3. Capabilities', 1);
      this.addContent('Four critical capabilities are integrated throughout the curriculum:');
      ...
      this.addContent('Key priorities that are addressed across learning areas:');
      this.addBulletList(data.crossCurriculumPriorities);
      this.doc.addPage();

      // Implementation Guidelines
      this.addHeader('6. Implementation Guidelines', 1);
      this.addContent('This curriculum is designed to be flexible and adaptable to meet the needs of all students. Teachers should:');
      this.addBulletList([
  ...
  static getSampleCurriculumData(): VCAACurriculumData {
    return {
      title: 'VCAA Curriculum F-10 Version 2.0',
      description: 'The Victorian Curriculum F-10 provides a single, coherent and comprehensive set of content descriptions and associated achievement standards to enable teachers to plan, monitor, assess and report on the learning achievement of every student.',
      content: 'Version 2.0 represents a significant update to the curriculum, focusing on contemporary learning needs, digital literacy, and preparing students for future challenges. The curriculum is structured around learning areas, capabilities, foundational skills, and cross-curriculum priorities.',
      learningAreas: [
