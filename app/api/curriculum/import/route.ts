...
function extractKeywords(text: string): string[] {
  ...
  const words = text.toLowerCase()
    ...
    .filter(word => !['that', 'this', 'with', 'from', 'they', 'have', 'been', 'were', 'said', 'each', 'which', 'their', 'time', 'will', 'about', 'would', 'there', 'could', 'other', 'after', 'first', 'well', 'also', 'new', 'want', 'because', 'any', 'these', 'give', 'day', 'may', 'say', 'use', 'her', 'many', 'some', 'very', 'when', 'much', 'then', 'them', 'can', 'only', 'think', 'come', 'over', 'also', 'back', 'after', 'use', 'two', 'how', 'our', 'work', 'first', 'well', 'way', 'even', 'new', 'want', 'because', 'any', 'these', 'give', 'day', 'may', 'say', 'use', 'her', 'many', 'some', 'very', 'when', 'much', 'then', 'them', 'can', 'only', 'think', 'come', 'over', 'also', 'back', 'after', 'use', 'two', 'how', 'our', 'work', 'first', 'well', 'way', 'even'].includes(word))
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    headers.forEach((header, index) => {
      ...
      if (fieldName && values[index]) {
        if (fieldName === 'topics') {
          outcome[fieldName] = values[index].split(';').map(t => t.trim()).filter(t => t)
        } else {
    ...
    if (outcome.learning_area && outcome.subject && outcome.level && outcome.content_description) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (outcomes.length === 0) {
      ...
      return NextResponse.json({ error: 'No valid curriculum outcomes found in file' }, { status: 400 })
    }
    ...
    for (let i = 0; i < outcomes.length; i += batchSize) {
      ...
      console.log(`Inserting batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(outcomes.length/batchSize)} (${batch.length} records)`)
      ...
      console.log(`Batch ${Math.floor(i/batchSize) + 1} inserted successfully. Total: ${totalInserted}`)
    }
