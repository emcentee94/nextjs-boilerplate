...
export default function TestCurriculumSimplePage() {
  ...
  const testDirectFetch = async () => {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        const itemsWithContent = data.filter(
          (item: any) =>
        ...
        setResult(
          `✅ Success! Loaded ${data.length} total items. Found ${itemsWithContent.length} items with content. First item with content: ${JSON.stringify(itemsWithContent[0] || data[0], null, 2)}`
        )
      } else {
        setResult(
          `⚠️ Data is not an array or is empty. Data: ${JSON.stringify(data, null, 2)}`
        )
      }
    } catch (err) {
  ...
  const testService = async () => {
    ...
    try {
      ...
      setResult(
        `✅ Service Success! Learning Areas: ${data.learning_areas.length}, Achievement Standards: ${data.achievement_standards.length}`
      )
    } catch (err) {
