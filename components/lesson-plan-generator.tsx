...
import type { LessonRequest, LessonPlan, CurriculumItem as LessonCurriculumItem } from "@/types/lesson-plan"
import { LEARNING_AREAS, listAllSubjects } from "@/src/data/curriculum/learningAreas"
...
export function LessonPlanGenerator({ onLessonPlanGenerated }: LessonPlanGeneratorProps) {
  const [subjects, setSubjects] = useState<string[]>([])
  const [yearLevels, setYearLevels] = useState<string[]>([])
  const [availableCurriculumItems, setAvailableCurriculumItems] = useState<CurriculumItem[]>([])
  const [selectedCurriculumItems, setSelectedCurriculumItems] = useState<LessonCurriculumItem[]>([])
  ...
  useEffect(() => {
    const loadCurriculumItems = async () => {
      if (formData.subject && formData.yearLevels?.length) {
        try {
          console.log("Loading curriculum items for:", formData.subject, formData.yearLevels[0])
          const items = await CurriculumService.getCurriculumItemsForSubjectAndLevel(
            formData.subject,
  ...
  const handleAddCurriculumItem = (item: CurriculumItem) => {
    ...
    if (code && !selectedCurriculumItems.find(selected => selected.code === code)) {
      setSelectedCurriculumItems(prev => [...prev, { 
        code, 
        title: item["Content Description"] || item.content_description || code 
      }])
    }
  }

  const handleRemoveCurriculumItem = (code: string) => {
    setSelectedCurriculumItems(prev => prev.filter(item => item.code !== code))
  }

  const handleGenerateLessonPlan = async () => {
    if (!formData.subject || !formData.yearLevels?.length || selectedCurriculumItems.length === 0) {
      alert("Please select a subject, year level, and at least one curriculum item.")
      return
    }
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <label htmlFor="trauma-informed" className="text-sm font-medium">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">Assessment Style</label>
                  <Select 
                    value={formData.options?.assessmentStyle || "formative"} 
                    onValueChange={(value: "formative" | "summative" | "rubric-upload" | "acara-standard" | "vcaa") => 
                      setFormData(prev => ({ 
                        ...prev, 
                        options: { 
                          ...prev.options!, 
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="rubric-upload">Rubric Upload</SelectItem>
                      <SelectItem value="acara-standard">ACARA Standard</SelectItem>
              ...
              <div className="space-y-2">
                ...
                <div className="max-h-60 overflow-y-auto space-y-2 border rounded-md p-3">
                  {availableCurriculumItems.slice(0, 20).map(item => {
                    const code = item["Code"] || item.code
                    const description = item["Content Description"] || item.content_description
                    const isSelected = selectedCurriculumItems.some(selected => selected.code === code)
                    
                    if (!code || isSelected) return null
                    
                    return (
                      <div key={code} className="flex items-start gap-2 p-2 hover:bg-muted/50 rounded">
                        <button
          ...
          <Button 
            ...
            disabled={isGenerating || !formData.subject || !formData.yearLevels?.length || selectedCurriculumItems.length === 0}
            className="w-full"
            size="lg"
          >
            {isGenerating ? (
