...
import { LESSON_PLAN_JSON_SCHEMA, type LessonPlan } from "../../../../src/domain/lessonPlan";
  ...
  for (const code of codes) {
    ...
    if (learningAreaItem) {
      results.push({
        ...
        achievementStandard: learningAreaItem["Achievement Standard"] || learningAreaItem.achievement_standard || `Achievement standard for ${code}`,
    ...
    if (achievementStandardItem) {
      results.push({
        ...
        achievementStandard: achievementStandardItem["Achievement Standard"] || achievementStandardItem.achievement_standard || `Achievement standard for ${code}`
      });
      continue;
    }
...
function buildUserMsg(req: LessonRequest, curriculum: any[]) {
  const { subject, yearLevels, items, durationMins, classProfile, options } = req;
  const lines: string[] = [];
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.subject || !body.yearLevels?.length || !body.durationMins || !body.options) {
      return NextResponse.json(
        { error: "Missing required fields: subject, yearLevels, durationMins, or options." },
        { status: 400 }
      );
    }
