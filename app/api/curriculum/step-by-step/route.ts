  ...
  try {
    ...
    for (const sheetName of workbook.SheetNames) {
      ...
      try {
        ...
        for (let i = 1; i < data.length; i++) {
          try {
            ...
            if (hasAnyContent) {
              ...
              const keywords = textForKeywords
                .toLowerCase()
                .replace(/[^\w\s]/g, ' ')
                .split(/\s+/)
              ...
              outcome.topics = [...(outcome.topics || []), ...keywords].slice(
                0,
                10
              )
            ...
            sheetErrors.push(
              `Row ${i}: ${rowError instanceof Error ? rowError.message : 'Unknown error'}`
            )
          }
        }

        console.log(
          `Step 4: Sheet ${sheetName} processed: ${sheetOutcomes} outcomes`
        )
        ...
        console.error(
          `Step 4: Error processing sheet ${sheetName}:`,
          sheetError
        )
        results.errors.push(
