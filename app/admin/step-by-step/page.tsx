...
export default function StepByStepPage() {
  ...
  const runStepByStep = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Step-by-step analysis failed')
    } finally {
      ...
      <div className="max-w-4xl mx-auto px-4">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Analyze each step of the import process to identify exactly where it's failing.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <Button 
              ...
              className="w-full"
            >
              {isAnalyzing ? 'Analyzing Step-by-Step...' : 'Run Step-by-Step Analysis'}
            </Button>
              ...
              <div className="space-y-6">
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <div className="text-green-700 space-y-2">
                    ...
                    <p><strong>Size:</strong> {(results.fileSize / 1024 / 1024).toFixed(2)} MB</p>
                    <p><strong>Sheets:</strong> {results.sheetCount}</p>
                    <p><strong>Total Outcomes:</strong> {results.totalOutcomes}</p>
                    <p><strong>Database Status:</strong> {results.databaseStatus}</p>
                    {results.currentRecords && (
                      <p><strong>Current Records:</strong> {results.currentRecords}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {results.sheets.map((sheet: any, index: number) => (
                    <Card key={index} className={sheet.error ? "bg-red-50" : "bg-gray-50"}>
                      <CardHeader>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {sheet.errors && sheet.errors.length > 0 && (
                          <div>
                            <h4 className="font-semibold mb-2 text-red-700">Row Errors:</h4>
                            <div className="bg-white p-3 rounded-md border border-red-200">
                              <ul className="text-red-600 text-sm space-y-1">
                                {sheet.errors.map((error: string, eIndex: number) => (
